...
func InstalledSoftwareList() ([]so.Software, error) {
	...
	swU, err := getUserSoftwareList(`Software\Microsoft\Windows\CurrentVersion\Uninstall`, "X32")
	if err != nil {
		return nil, err
	}
...
func getUserSoftwareList(baseKey string, arch string) ([]so.Software, error) {
	k, err := registry.OpenKey(registry.CURRENT_USER, baseKey, registry.QUERY_VALUE|registry.ENUMERATE_SUB_KEYS)
	if err != nil {
	...
	for _, sw := range subkeys {
		sk, err := registry.OpenKey(registry.CURRENT_USER, baseKey+`\`+sw, registry.QUERY_VALUE)
		if err != nil {
